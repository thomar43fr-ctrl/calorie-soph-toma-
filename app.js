...
// ── Upload photo (base64 local, pas de Storage) ──────────────
...
function renderIngredients() {
  ...
  container.innerHTML = `
    ...
    <div class="search-filter-row">
      <input id="ing-search" type="text" placeholder="Rechercher un ingrédient..." value="${search}" oninput="renderIngredients()" class="input-search">
      <select id="ing-filter" onchange="renderIngredients()" class="input-select">
        <option value="all">Toutes catégories</option>
        ${INGREDIENT_CATEGORIES.map(c => `<option value="${c}" ${filterCat === c ? 'selected' : ''}>${c}</option>`).join('')}
      </select>
    </div>
    <div class="ing-list">
      ${filtered.length === 0 ? '<p class="empty-hint">Aucun ingrédient trouvé</p>' : categoriesHTML}
    </div>`;
}
